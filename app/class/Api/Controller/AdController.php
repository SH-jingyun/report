...
//	Banner:banner
        ...
        $sql = 'SELECT position_id, ad_source, ad_source_code, position_type, strategy_id FROM ad_position LEFT JOIN ad_strategy USING(position_id) LEFT JOIN ad_position_type USING(position_type_id) LEFT JOIN ad_ad_code USING(strategy_id) WHERE app_id = 10003 AND group_id = 1';
//        $prepareSql = $this->locator->db2->prepare($sql);
//        $prepareSql->execute($dataArr);
//        $result = $prepareSql->fetchAll(\PDO::FETCH_GROUP);
        $list = $this->locator->db->prepare->fetchAll(\PDO::FETCH_GROUP);
        var_dump($list);
